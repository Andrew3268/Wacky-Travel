...
export function renderHotelReviewBlock(block = {}) {
  switch (text(block.type)) {
    ...
    case "locationSummary":
      return `<div class="hrj-location-summary" data-hrj-location-summary>
        <div class="hrj-location-summary__head">
          <h3>${escapeHtml(text(block.title || "위치를 한눈에 보면"))}</h3>
          <button class="hrj-location-summary__copy" type="button" data-hrj-location-summary-copy aria-label="위치 요약 복사" title="위치 요약 복사">
            <svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="7" y="3" width="12" height="14" rx="3" stroke="currentColor" stroke-width="1.8"/><rect x="3" y="7" width="12" height="14" rx="3" fill="currentColor" fill-opacity="0" stroke="currentColor" stroke-width="1.8"/></svg>
          </button>
        </div>
...
export function renderHotelReviewLocationMap(data = {}) {
  ...
  return `<section class="hrj-location-map" data-hrj-location-map data-map-config="${safeConfig}" aria-label="호텔 주변 지도">
    ...
    <div class="hrj-map-canvas-wrap">
      ...
      <div class="hrj-map-overlay" data-hrj-map-overlay hidden aria-live="polite" aria-label="선택한 장소 정보">
        <button type="button" class="hrj-map-overlay__close" data-hrj-map-overlay-close aria-label="장소 정보 닫기">×</button>
        <div class="hrj-map-overlay__name" data-hrj-map-overlay-name></div>
        <div class="hrj-map-overlay__meta" data-hrj-map-overlay-meta></div>
      </div>
    </div>
...
export function renderHotelReviewSections(data = {}) {
  ...
  return array(data.sections).map((section) => {
    ...
    const body = isLocation ? renderHotelReviewLocationBlocks(blocks) : blocks.map((block) => {
      const type = text(block?.type);
    ...
    return `
    <section class="hrj-chapter${isLocation ? " hrj-chapter--location" : ""}" id="${escapeHtml(sectionId)}">
...
export function renderHotelReviewLayout(data = {}, options = {}) {
  ...
  const checkInOutValue = findBasicInfoValue("체크인·체크아웃", "체크인/체크아웃", "체크인");
  const checkInOutParts = checkInOutValue
    .split(/\s*(?:\/|·|\||→|-)\s*/)
    .map((value) => text(value))
    .filter(Boolean);
  const breakfastValue = findBasicInfoValue("조식") || "예약 요금제 확인";
  const locationValue = findBasicInfoValue("위치") || text(hotel.area) || text(hotel.city);
  const cancellationValue = findBasicInfoValue("무료 취소", "취소");
  const airportValue = text(hotel.airport) || findBasicInfoValue("공항");
  const airportTime = airportValue.match(/차량\s*약?\s*\d+\s*분/i)?.[0] || "";

  const overviewFacts = checkInOutParts.length >= 2
    ? [
        { label: "체크인", value: checkInOutParts[0] },
        { label: "체크아웃", value: checkInOutParts[1] },
        { label: "조식", value: breakfastValue }
      ]
    : [
        { label: "위치", value: locationValue },
        { label: "체크인·체크아웃", value: checkInOutValue || "예약 정보 확인" },
        { label: "조식", value: breakfastValue }
      ];

  const overviewTags = Array.from(new Set([
    text(hotel.grade),
    text(hotel.roomCount),
    airportTime ? `공항 ${airportTime}` : "",
    cancellationValue
  ].filter(Boolean)));

  const overviewEyebrow = [text(hotel.country), text(hotel.city), text(hotel.area)].filter(Boolean).join(" · ");
  const overviewDescription = text(hotel.type)
    ? `${escapeHtml(text(hotel.type))}입니다. 숙소 선택 전에 빠르게 확인하면 좋은 객관적인 정보만 정리했습니다.`
    : "숙소 선택 전에 빠르게 확인하면 좋은 객관적인 정보만 정리했습니다.";
        ...
        <div class="hrj-article__inner">
          ...
          <section class="hrj-booking-overview" aria-label="예약 전 확인할 기본 정보">
            ...
            <div class="hrj-basic-info">
              ...
              <p class="hrj-basic-info__desc">${overviewDescription}</p>
              <div class="hrj-basic-info__grid">${overviewFacts.map((item) => `<div class="hrj-basic-info__item"><span class="hrj-basic-info__label">${escapeHtml(text(item.label))}</span><span class="hrj-basic-info__value">${escapeHtml(text(item.value))}</span></div>`).join("")}</div>
              ${overviewTags.length ? `<div class="hrj-basic-info__tags">${overviewTags.map((tag, index) => `<span class="hrj-basic-info__tag${index === 0 ? " is-accent" : ""}">${escapeHtml(tag)}</span>`).join("")}</div>` : ""}
              ${basicInfoCta}
            </div>
          </section>
