...
function locationMapConfig(data = {}) {
  ...
  const categories = array(map.categories).filter((category) => isObject(category) && array(category.items).length);
...
function mapDistanceLabel(item = {}) {
...
function mapTravelLabel(item = {}) {
  const bits = [];
  const walk = number(item?.travel?.walkMinutes);
  const drive = number(item?.travel?.driveMinutes);
...
function renderLocationMapListItem(item = {}, categoryKey = "") {
  const distance = mapDistanceLabel(item);
  const travel = mapTravelLabel(item);
  const meta = [text(item.nameLocal) || text(item.nameEn), distance, travel].filter(Boolean).join(" · ");
  ...
  return `<button class="hrj-map-place" type="button" data-hrj-map-place="${escapeHtml(text(item.id))}" data-map-category="${escapeHtml(text(categoryKey))}">
    ...
    <span class="hrj-map-place__body"><strong>${escapeHtml(text(item.nameKo))}</strong>${meta ? `<span>${escapeHtml(meta)}</span>` : ""}</span>
...
export function renderHotelReviewSections(data = {}) {
  ...
  return array(data.sections).map((section) => {
    ...
    const body = blocks.map((block) => {
      const rendered = renderHotelReviewBlock(block);
      if (!insertedMap && mapHtml && text(section?.id) === "attractions-transport" && text(block?.type) === "accessSummary") {
        insertedMap = true;
        return `${rendered}${mapHtml}`;
      }
      return rendered;
    }).join("");
    const fallbackMap = !insertedMap && mapHtml && text(section?.id) === "attractions-transport" ? mapHtml : "";
    return `
