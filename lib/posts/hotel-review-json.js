...
import { renderTripcomSidebarIframe } from "./tripcom-sidebar-ad.js";
...
function deriveSidebarFitTags(data = {}) {
  const fitGrid = findHotelReviewBlock(data, "fitGrid");
  const tags = [];
  const add = (label) => {
    const value = text(label);
    if (value && !tags.includes(value) && tags.length < 3) tags.push(value);
  };

  for (const item of array(fitGrid?.good)) {
    const raw = text(item);
    if (!raw) continue;
    if (/(가족|아이|키즈)/.test(raw)) add("가족여행");
    if (/자유여행/.test(raw)) add("자유여행");
    if (/(여러 지역|두루)/.test(raw)) add("여러 지역 여행");
    if (/시내/.test(raw) && /해변/.test(raw)) add("시내+해변");
    if (/(리버뷰|전망)/.test(raw)) add("전망 중시");
    if (/공항/.test(raw)) add("공항 이동 편의");
...
function deriveSidebarChecks(data = {}) {
  const quickPoints = array(data?.article?.quickPoints);
  const positivePreference = ["객실 선택 포인트", "이동 방식", "가족 여행", "위치 성격", "생활 편의"];
  const positives = [];

  for (const label of positivePreference) {
    const item = quickPoints.find((entry) => text(entry?.label) === label);
    if (item && text(item.value) && !positives.includes(text(item.value))) positives.push(text(item.value));
    if (positives.length >= 2) break;
  }
  if (positives.length < 2) {
    for (const item of quickPoints) {
      const label = text(item?.label);
      const value = text(item?.value);
      if (!value || /(주의|단점|아쉬)/.test(label) || positives.includes(value)) continue;
      positives.push(value);
      if (positives.length >= 2) break;
    }
  }

  const cautionPoint = quickPoints.find((item) => /(주의|단점|아쉬)/.test(text(item?.label)) && text(item?.value));
  const fitGrid = findHotelReviewBlock(data, "fitGrid");
  const caution = text(cautionPoint?.value) || text(array(fitGrid?.bad)[0]);

  return [
    ...positives.slice(0, 2).map((value) => ({ type: "good", text: value })),
    ...(caution ? [{ type: "warn", text: caution }] : [])
  ];
}

function deriveSidebarRoomPicks(data = {}) {
  const roomOptions = findHotelReviewBlock(data, "roomOptions");
  return array(roomOptions?.items)
    .filter((item) => text(item?.title) && text(item?.desc))
    .slice(0, 2)
    .map((item) => ({ title: text(item.title), desc: text(item.desc) }));
}

function renderHotelDecisionCard(data = {}, options = {}) {
  const hotel = isObject(data.hotel) ? data.hotel : {};
  const fitTags = deriveSidebarFitTags(data);
  const checks = deriveSidebarChecks(data);
  const roomPicks = deriveSidebarRoomPicks(data);
  const availabilityUrl = safeHttpUrl(options.availabilityUrl);
  const cta = availabilityUrl
    ? `<a class="hrj-side-cta hrj-decision-card__cta" href="${escapeHtml(availabilityUrl)}" target="_blank" rel="sponsored noopener noreferrer">객실·요금 확인하기</a>`
    : "";

  const fitSection = fitTags.length
    ? `<section class="hrj-decision-section"><div class="hrj-decision-section__title">이런 여행에 잘 맞아요</div><div class="hrj-decision-tags">${fitTags.map((tag) => `<span class="hrj-decision-tag">${escapeHtml(tag)}</span>`).join("")}</div></section>`
    : "";

  const checkSection = checks.length
    ? `<section class="hrj-decision-section"><div class="hrj-decision-section__title">예약 전 체크</div><div class="hrj-decision-checks">${checks.map((item) => `<div class="hrj-decision-check${item.type === "warn" ? " hrj-decision-check--warn" : ""}"><span class="hrj-decision-check__icon" aria-hidden="true">${item.type === "warn" ? "!" : "✓"}</span><span>${escapeHtml(item.text)}</span></div>`).join("")}</div></section>`
    : "";

  const roomSection = roomPicks.length
    ? `<section class="hrj-decision-section"><div class="hrj-decision-section__title">객실 선택 포인트</div><div class="hrj-decision-rooms">${roomPicks.map((item) => `<div class="hrj-decision-room"><div><div class="hrj-decision-room__need">${escapeHtml(item.desc)}</div><div class="hrj-decision-room__name">${escapeHtml(item.title)}</div></div><span class="hrj-decision-room__arrow" aria-hidden="true">›</span></div>`).join("")}</div></section>`
    : "";

  return `<div class="hrj-sidecard hrj-decision-card">
    <div class="hrj-decision-card__hotel">${escapeHtml(text(hotel.nameKo))}</div>
    ${fitSection}
    ${checkSection}
    ${roomSection}
    ${cta}
    ${cta ? `<div class="hrj-decision-card__microcopy">객실 타입과 요금은 예약 시점에 따라 달라질 수 있습니다.</div>` : ""}
  </div>`;
}
...
export function renderHotelReviewLayout(data = {}, options = {}) {
  ...
  const availabilityUrl = safeHttpUrl(options.availabilityUrl);
  const decisionCard = renderHotelDecisionCard(data, { availabilityUrl });
  const tripcomSidebarIframe = renderTripcomSidebarIframe(options.tripcomSidebarAdUrl);
  const primarySidebarCard = tripcomSidebarIframe
    ? `<div class="hrj-sidecard hrj-sidecard--tripcom">${tripcomSidebarIframe}</div>`
    : decisionCard;

  return `
  <main id="main-content" class="hotel-review-json-page${tripcomSidebarIframe ? " has-tripcom-sidebar" : ""}">
    ...
    <div class="hrj-shell">
      <article class="hrj-article">
        <div class="hrj-article__inner">
          ...
          <nav class="hrj-mobile-toc" aria-label="이 글의 목차"><div class="hrj-mobile-toc__title">이 글의 목차</div><ul>${toc}</ul></nav>
