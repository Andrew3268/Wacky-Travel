...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  function flushParagraph() {
    ...
    pushBlock("paragraph", `<p>${inlineFormat(paragraph.join("\n"), contentLinkSettings)}</p>`);
    paragraph = [];
  }
  ...
  function flushBlockquote() {
    ...
    pushBlock("blockquote", `<blockquote><p>${inlineFormat(blockquoteLines.join("\n"), contentLinkSettings)}</p></blockquote>`);
    inBlockquote = false;
    blockquoteLines = [];
  }
  ...
  function flushFencedCode() {
    ...
    pushBlock("code", renderFencedCodeHtml(fencedCodeLines, activeCodeFence.language));
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (activeCodeFence) {
      if (isFencedCodeClose(trimmed, activeCodeFence.fenceLength)) {
        flushFencedCode();
      } else {
    ...
    const editorTip = readEditorTip(lines, index);
    if (editorTip) {
      flushParagraph(); flushList(); flushOrdered(); flushBlockquote();
      const tipHtml = renderEditorTip(editorTip.lines);
      if (tipHtml) pushBlock("editor-tip", tipHtml);
      index = editorTip.end;
      continue;
    }

    const codeFenceOpen = parseFencedCodeOpen(trimmed);
    if (codeFenceOpen) {
      flushParagraph();
      flushList();
      flushOrdered();
