...
export function parseAffiliateCta(md = "") {
  ...
  String(md || "").split("\n").forEach((line) => {
    ...
    if (!token) return;
    result = {
      enabled: !!(token.buttonText || token.linkUrl),
...
export function renderAffiliateCtaHtml(data = {}) {
  ...
  return `
    <div class="post-affiliate-cta" aria-label="제휴 마케팅 CTA">
      <span class="post-affiliate-cta__label">제휴 링크</span>
      ${data.linkUrl ? `<a class="post-affiliate-cta__button" href="${escapeHtml(data.linkUrl)}" target="_blank" rel="nofollow sponsored noopener noreferrer">${escapeHtml(buttonText)}</a>` : `<span class="post-affiliate-cta__button is-disabled">${escapeHtml(buttonText)}</span>`}
    </div>
  `;
}

function insertAffiliateCtaAtTargetH2SectionEnd(blocks = [], affiliateCta = {}) {
  if (!affiliateCta?.enabled || !(affiliateCta.buttonText || affiliateCta.linkUrl)) return blocks;
  ...
  if (targetHeadingIndex < 0) return blocks;

  let insertIndex = blocks.length;
  for (let index = targetHeadingIndex + 1; index < blocks.length; index += 1) {
    const block = blocks[index];
    if (block?.type === "heading" && block.level === 2) {
      insertIndex = index;
      break;
  ...
  const ctaHtml = renderAffiliateCtaHtml(affiliateCta);
  if (!ctaHtml) return blocks;
  return [
    ...blocks.slice(0, insertIndex),
    { type: "affiliate-cta", html: ctaHtml },
...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  const affiliateCta = options.affiliateCta || parseAffiliateCta(normalizedMd);
  let h2Count = 0;
    ...
    if (heading) {
      ...
      if (level === 2) {
        ...
        if (h2Count === image1Target && inlineImages.image1?.enabled && (inlineImages.image1?.url || inlineImages.image1?.id)) {
          pushBlock("inline-image", renderInlineImageHtml(inlineImages.image1, options));
        }
        if (h2Count === image2Target && inlineImages.image2?.enabled && (inlineImages.image2?.url || inlineImages.image2?.id)) {
          pushBlock("inline-image", renderInlineImageHtml(inlineImages.image2, options));
        }
      }
      continue;
    }
  ...
  return insertAffiliateCtaAtTargetH2SectionEnd(blocks, affiliateCta);
}
