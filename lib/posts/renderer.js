...
function renderStyleHotelImageHtml(data = {}, options = {}) {
  ...
  if (source === "agoda") {
    ...
    const linked = isAgodaPartnerLink(link)
      ...
      : image;
    return `<figure class="post-inline-image post-inline-image--agoda post-style-hotel-image">${linked}</figure>${renderStyleHotelMetaHtml(data)}`;
  }
  ...
  return `<figure class="post-inline-image post-style-hotel-image"><img class="post-inline-image__img" ${image.attrs} alt="${escapeHtml(alt)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" /></figure>${renderStyleHotelMetaHtml(data)}`;
}
