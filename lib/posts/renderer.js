...
import { getContentLinkType } from "./content-link-settings.js";
...
function parseStyleHotelImageToken(line = "") {
  ...
  return {
    ...
    starRating: decodeInlineTokenValue(attrs.star || "").trim(),
    guestRating: decodeInlineTokenValue(attrs.rating || "").trim(),
    badge: decodeInlineTokenValue(attrs.badge || "").trim(),
    imageBadge: decodeInlineTokenValue(attrs.image_badge || attrs.imageBadge || "").trim()
  };
}
...
function parseStyleHotelButtonToken(line = "") {
  ...
  const attrs = parseTokenAttributes(match[1]);
  return {
    buttonText: decodeInlineTokenValue(attrs.text || attrs.button || "잔여 객실 확인").trim() || "잔여 객실 확인",
...
function markStyleHotelImageBeforeSectionLabelBlock(block = {}) {
  const html = String(block.html || "");
  return {
    ...block,
    html: html.replace(/^<figure\s+class="([^"]*)"/i, (_, classNames = "") => {
      const mergedClasses = Array.from(new Set([
        ...String(classNames || "").split(/\s+/).filter(Boolean),
        "post-inline-image--before-section-label"
      ])).join(" ");
      return `<figure class="${mergedClasses}"`;
    })
  };
}

function moveStyleHotelSectionDividerToImage(blocks = [], options = {}) {
  ...
  for (let index = 1; index < result.length - 1; index += 1) {
    ...
    const previousBlock = result[index - 1];
    const nextBlock = result[index + 1];
    if (!isHotelReviewSectionLabelBlock(block)) continue;
    if (previousBlock?.type !== "style-hotel-image") continue;
    if (!nextBlock || nextBlock.type !== "heading" || nextBlock.level !== 2) continue;
    result[index - 1] = markStyleHotelImageBeforeSectionLabelBlock(previousBlock);
    result[index] = markHotelReviewSectionLabelAfterImageBlock(block);
  }
  return result;
}

function normalizeStyleHotelBadges(raw = "") {
...
function renderStyleHotelMetaHtml(data = {}) {
  ...
  const rating = /^(?:6\.5|7\.0|7\.5|8\.0|8\.5|9\.0|9\.5)\+$/.test(String(data.guestRating || "").trim())
    ...
    : "";
  const items = [];
  if (star) items.push(`<span class="post-style-hotel-meta__item post-style-hotel-meta__item--star">${escapeHtml(star)}</span>`);
  if (star && rating) items.push('<span class="post-style-hotel-meta__separator" aria-hidden="true">|</span>');
  if (rating) items.push(`<span class="post-style-hotel-meta__item post-style-hotel-meta__item--rating"><span aria-hidden="true">★</span>${escapeHtml(rating)}</span>`);
  return items.length
    ? `<div class="post-style-hotel-meta" aria-label="호텔 성급 및 평점">${items.join("")}</div>`
    : "";
}

function renderStyleHotelBadgeHtml(data = {}) {
  const badges = normalizeStyleHotelBadges(data.badge);
  if (!badges.length) return "";
  return `<p class="post-style-hotel-badges" aria-label="호텔 특징">${badges.map((badge) => escapeHtml(badge)).join('<span class="post-style-hotel-badges__separator" aria-hidden="true">|</span>')}</p>`;
}

function renderStyleHotelImageBadgeHtml(data = {}) {
  const text = String(data.imageBadge || "").replace(/\s+/g, " ").trim().slice(0, 40);
  return text ? `<span class="post-style-hotel-image-badge">${escapeHtml(text)}</span>` : "";
}
...
function renderStyleHotelImageHtml(data = {}, options = {}) {
  ...
  if (source === "agoda") {
    ...
    const linked = isAgodaPartnerLink(link)
      ...
      : image;
    return `<figure class="post-inline-image post-inline-image--agoda post-style-hotel-image">${linked}${renderStyleHotelImageBadgeHtml(data)}</figure>${renderStyleHotelMetaHtml(data)}`;
  }
  ...
  return `<figure class="post-inline-image post-style-hotel-image"><img class="post-inline-image__img" ${image.attrs} alt="${escapeHtml(alt)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" />${renderStyleHotelImageBadgeHtml(data)}</figure>${renderStyleHotelMetaHtml(data)}`;
}

function renderStyleHotelButtonHtml(data = {}) {
  const text = String(data.buttonText || "잔여 객실 확인").trim() || "잔여 객실 확인";
...
function insertStyleHotelBadgesBelowHeadings(blocks = [], options = {}) {
  if (options.hotelSectionHeadingClasses !== true) return blocks;
  const result = blocks.map((block) => ({ ...block }));

  for (let imageIndex = 0; imageIndex < result.length; imageIndex += 1) {
    const imageBlock = result[imageIndex];
    if (imageBlock?.type !== "style-hotel-image") continue;

    let headingIndex = -1;
    for (let index = imageIndex + 1; index < result.length; index += 1) {
      const candidate = result[index];
      if (candidate?.type === "style-hotel-image" || candidate?.type === "style-hotel-button") break;
    ...
    if (result[headingIndex + 1]?.type === "list") {
      result.splice(headingIndex + 1, 1);
    }

    const badgeHtml = renderStyleHotelBadgeHtml(imageBlock.styleHotel || {});
    if (badgeHtml) {
      result.splice(headingIndex + 1, 0, {
        type: "style-hotel-badges",
        html: badgeHtml
      });
      imageIndex += 1;
    }
...
function inlineFormat(text = "", contentLinkSettings = []) {
  return escapeHtml(text)
    ...
    .replace(/\[([^\]]+)\]\((https?:[^\s)]+)\)/g, (match, linkText, linkUrl) => {
      const rawText = String(linkText || "").replace(/&amp;/g, "&").replace(/&quot;/g, '"').replace(/&#39;/g, "'");
      const rawUrl = String(linkUrl || "").replace(/&amp;/g, "&");
      const type = getContentLinkType(contentLinkSettings, rawText, rawUrl);
      const rel = type === "affiliate" ? "sponsored nofollow noopener" : "noopener";
      const className = type === "affiliate"
        ? "post-content-link post-content-link--affiliate"
        : "post-content-link post-content-link--normal";
      return `<a class="${className}" href="${linkUrl}" target="_blank" rel="${rel}">${linkText}</a>`;
    })
...
function renderHeadingText(level, text = "", contentLinkSettings = []) {
  const normalized = normalizeArticleHeadingText(level, text);
  if (level !== 2) return inlineFormat(normalized, contentLinkSettings);

  const raw = normalized;
  const boldPrefixMatch = raw.match(/^\*\*\s*([^*：:]{1,30}?)\s*[:：]\s*\*\*\s*(.+)$/);
  if (boldPrefixMatch) {
    const prefix = `${boldPrefixMatch[1].trim()}:`;
    return `<span class="post-h2-prefix"><strong>${inlineFormat(prefix, contentLinkSettings)}</strong></span><span class="post-h2-text">${inlineFormat(boldPrefixMatch[2], contentLinkSettings)}</span>`;
  }

  const match = raw.match(/^([^:：]{1,30}?)\s*[:：]\s*(.+)$/);
  if (!match) {
    return `<span class="post-h2-text">${inlineFormat(raw, contentLinkSettings)}</span>`;
  }

  const prefix = `${match[1].trim()}:`;
  return `<span class="post-h2-prefix">${inlineFormat(prefix, contentLinkSettings)}</span><span class="post-h2-text">${inlineFormat(match[2], contentLinkSettings)}</span>`;
}
...
function renderMarkdownTableFromLines(lines = [], contentLinkSettings = []) {
  ...
  const thead = `<thead><tr>${headerCells.map((cell, idx) => {
    ...
    return `<th class="table-cell${alignClass}">${inlineFormat(cell, contentLinkSettings)}</th>`;
  }).join("")}</tr></thead>`;
  ...
  const tbody = bodyRows.length
    ? `<tbody>${bodyRows.map((row) => `<tr>${headerCells.map((_, idx) => {
        ...
        const alignClass = align ? ` table-cell--${align}` : "";
        const cell = row[idx] ?? "";
        return `<td class="table-cell${alignClass}">${inlineFormat(cell, contentLinkSettings)}</td>`;
      }).join("")}</tr>`).join("")}</tbody>`
    : "";
...
export function renderTocHtml(items = [], mode = "h2", options = {}) {
...
function renderMarkdownCallout(titleMatch, bodyLines = [], contentLinkSettings = []) {
  if (!titleMatch || !bodyLines.length) return "";
  const marker = titleMatch[1] || "";
  const title = titleMatch[2] || "";
  return `<aside class="post-markdown-callout" role="note">
    <div class="post-markdown-callout__title"><span class="post-markdown-callout__marker" aria-hidden="true">${escapeHtml(marker)}</span><span>${inlineFormat(title, contentLinkSettings)}</span></div>
    <div class="post-markdown-callout__body">${bodyLines.map((item) => `<div class="post-markdown-callout__line">${inlineFormat(item, contentLinkSettings)}</div>`).join("")}</div>
  </aside>`;
}

function renderMarkdownCheckLines(items = [], contentLinkSettings = []) {
  if (!items.length) return "";
  return `<div class="post-markdown-checklines">${items.map((item) => `<div class="post-markdown-checklines__item"><span class="post-markdown-checklines__marker" aria-hidden="true">${escapeHtml(item.marker)}</span><span>${inlineFormat(item.text, contentLinkSettings)}</span></div>`).join("")}</div>`;
}
...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  const contentLinkSettings = options.contentLinkSettings || [];
  let h2Count = 0;
  ...
  function flushParagraph() {
    ...
    pushBlock("paragraph", `<p>${inlineFormat(paragraph.join(" "), contentLinkSettings)}</p>`);
    paragraph = [];
  }

  function flushList() {
    if (!listItems.length) return;
    pushBlock("list", `<ul>${listItems.map((item) => `<li>${inlineFormat(item, contentLinkSettings)}</li>`).join("")}</ul>`);
    listItems = [];
  }

  function flushOrdered() {
    if (!orderedItems.length) return;
    pushBlock("ordered", `<ol>${orderedItems.map((item) => `<li>${inlineFormat(item, contentLinkSettings)}</li>`).join("")}</ol>`);
    orderedItems = [];
  }

  function flushBlockquote() {
    if (!inBlockquote) return;
    pushBlock("blockquote", `<blockquote><p>${inlineFormat(blockquoteLines.join(" "), contentLinkSettings)}</p></blockquote>`);
    inBlockquote = false;
    blockquoteLines = [];
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const calloutTitle = matchMarkdownCalloutTitle(trimmed);
    if (calloutTitle) {
      ...
      while (rowIndex < lines.length) {
        const bodyTrimmed = String(lines[rowIndex] || "").trim();
        if (!bodyTrimmed) break;
        if (/^(#{1,4})\s+/.test(bodyTrimmed) || /^>\s?/.test(bodyTrimmed) || /^[-*]\s+/.test(bodyTrimmed) || /^\d+\.\s+/.test(bodyTrimmed)) break;
        if (parseInlineImageToken(bodyTrimmed) || parseAffiliateToken(bodyTrimmed) || parseAffiliateCtaToken(bodyTrimmed) || parseTocMode(bodyTrimmed)) break;
        bodyLines.push(bodyTrimmed);
        rowIndex += 1;
      }
      if (bodyLines.length) {
        flushParagraph();
        ...
        flushOrdered();
        flushBlockquote();
        pushBlock("callout", renderMarkdownCallout(calloutTitle, bodyLines, contentLinkSettings));
        index = rowIndex - 1;
        continue;
      }
    }

    const checkLine = matchMarkdownCheckLine(trimmed);
    if (checkLine) {
      const items = [];
      let rowIndex = index;
      while (rowIndex < lines.length) {
      ...
      flushOrdered();
      flushBlockquote();
      pushBlock("checklines", renderMarkdownCheckLines(items, contentLinkSettings));
      index = rowIndex - 1;
      continue;
    }
    ...
    if (styleHotelImage) {
      ...
      if (imageHtml) pushBlock("style-hotel-image", imageHtml, { styleHotel: styleHotelImage });
      continue;
    }
    ...
    if (STYLE_HOTEL_ENDING_TOKEN_RE.test(trimmed)) {
      ...
      if (!styleHotelEndingOpen) {
        pushBlock("style-hotel-ending-open", '<section class="post-style-hotel-ending">');
    ...
    if (
      ...
      pushBlock("table", renderMarkdownTableFromLines(tableLines, contentLinkSettings));
      index = rowIndex - 1;
      continue;
    }
    ...
    if (heading) {
      ...
      const hotelMagazineH3Line = level === 3 && (
        ...
        : "";
      pushBlock("heading", `<h${level} id="${escapeHtml(id)}">${hotelMagazineH3Line}${renderHeadingText(level, text, contentLinkSettings)}</h${level}>`, { level, text, id });
      continue;
    }
  ...
  flushBlockquote();
  if (styleHotelEndingOpen) {
    pushBlock("style-hotel-ending-close", "</section>");
  }
  ...
  const blocksWithStyleHotelImageDividers = moveStyleHotelSectionDividerToImage(blocksWithHotelSectionClasses, options);
  const blocksWithStyleHotelBadges = insertStyleHotelBadgesBelowHeadings(blocksWithStyleHotelImageDividers, options);
  return insertInlineImagesAtH2Anchors(blocksWithStyleHotelBadges, inlineImages, options);
}
