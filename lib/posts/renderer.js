...
function renderStyleHotelMetaHtml(data = {}) {
  ...
  if (badge) items.push(`<span class="post-style-hotel-meta__item post-style-hotel-meta__item--badge">${escapeHtml(badge)}</span>`);
  return items.length
    ? `<div class="post-style-hotel-meta" aria-label="호텔 성급, 평점 및 특징">${items.join("")}</div>`
    : "";
}
...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  return insertInlineImagesAtH2Anchors(blocksWithStyleHotelImageDividers, inlineImages, options);
}
