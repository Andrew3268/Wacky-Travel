...
function markHotelReviewSectionLabelAfterImageBlock(block = {}) {
  ...
  if (/^<p\s+class=/i.test(html.trim())) {
    return {
      ...block,
      html: html.replace(/^<p\s+class="([^"]*)"/i, '<p class="$1 post-section-label post-section-label--after-image"')
    };
  }
  return {
    ...block,
...
function isHotelReviewSectionLabelBlock(block = {}) {
  ...
  const match = String(block.html || "").trim().match(/^<p>\s*<strong>([\s\S]+?)<\/strong>\s*<\/p>$/i);
...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  return insertInlineImagesAtH2Anchors(blocksWithAffiliateCtas, inlineImages, options);
}
