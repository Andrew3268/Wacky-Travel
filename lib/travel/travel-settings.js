...
export const DEFAULT_DESTINATIONS = [
  ...
  { slug: "ho-chi-minh-city", name: "호찌민시", city: "호찌민시", country_slug: "vietnam", sort_order: 3 },
...
export async function ensureTravelSettingsTables(db) {
  ...
  if (shouldRunCountryCitySeed) {
    ...
    for (const item of DEFAULT_DESTINATIONS) {
      ...
      const metaDescription = `${countryName} ${item.name} 여행과 숙소 정보를 한눈에 정리한 와키트래블 기본 도시 항목입니다.`;
