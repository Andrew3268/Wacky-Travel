...
export const DEFAULT_DESTINATIONS = [
  ...
  { slug: "bangkok", name: "방콕", city: "방콕", country_slug: "thailand", sort_order: 1 },
  { slug: "pattaya", name: "파타야", city: "파타야", country_slug: "thailand", sort_order: 2 },
  { slug: "phuket", name: "푸켓", city: "푸켓", country_slug: "thailand", sort_order: 3 },
  { slug: "chiang-mai", name: "치앙마이", city: "치앙마이", country_slug: "thailand", sort_order: 4 },
  { slug: "koh-samui", name: "코사무이", city: "코사무이", country_slug: "thailand", sort_order: 5 },

  { slug: "cebu", name: "세부", city: "세부", country_slug: "philippines", sort_order: 1 },
  { slug: "boracay", name: "보라카이", city: "보라카이", country_slug: "philippines", sort_order: 2 },
  { slug: "bohol", name: "보홀", city: "보홀", country_slug: "philippines", sort_order: 3 },
  { slug: "manila", name: "마닐라", city: "마닐라", country_slug: "philippines", sort_order: 4 },
  { slug: "clark", name: "클락", city: "클락", country_slug: "philippines", sort_order: 5 },
...
export async function ensureTravelContentRelationColumns(db) {
  ...
  // /admin/items and /admin/home-hub screens.
...
export async function getTravelSettings(db, { includeInactive = true } = {}) {
  ...
  // D1 executes queries against one database session. Sequential reads avoid
  // one failed concurrent statement cancelling the whole settings response.
  const contentTypeRows = await db.prepare(`
  ...
  const countryRows = await db.prepare(`
    ...
    `).all();
  const destinationRows = await db.prepare(`
  ...
  const regionRows = await db.prepare(`
      ...
      LIMIT 1000
    `).all();
  const recommendationCategoryRows = await db.prepare(`
