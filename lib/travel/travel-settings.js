...
export async function ensureTravelSettingsTables(db) {
  ...
  await db.prepare(`CREATE INDEX IF NOT EXISTS idx_recommendation_categories_country_destination ON recommendation_categories(country_slug, destination_slug, sort_order ASC, name ASC)`).run();
  ...
  const recommendationCount = await db.prepare(`SELECT COUNT(*) AS count FROM recommendation_categories`).first();
  if (Number(recommendationCount?.count || 0) === 0) {
    const osakaDestination = await db.prepare(`SELECT slug, country FROM destinations WHERE slug = 'osaka' LIMIT 1`).first();
    if (osakaDestination?.slug) {
      const countrySlug = countryToSlug(osakaDestination.country || '일본') || 'japan';
      for (const item of DEFAULT_RECOMMENDATION_CATEGORIES) {
        await db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
        `).bind(item.slug, item.name, item.description, countrySlug, osakaDestination.slug, item.sort_order || 0, now, now).run();
      }
    }
  }
}
...
export async function getTravelSettings(db, { includeInactive = true } = {}) {
  ...
  const recommendationCategories = (recommendationCategoryRows.results || []).map((category) => {
    const destination = destinationBySlug.get(String(category.destination_slug || ""));
    return {
      ...category,
      country_slug: category.country_slug || destination?.country_slug || "",
      country_name: countryBySlug.get(category.country_slug || destination?.country_slug || "")?.name || destination?.country_name || "",
      destination_name: destination?.name || destination?.city || category.destination_slug || ""
    };
  });
