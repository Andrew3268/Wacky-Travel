export const DEFAULT_SITE_ORIGIN = "https://bestayable.com";
...
export function getSiteOrigin(env = {}, request = null) {
  ...
  const customDomainConnected = String(env?.CUSTOM_DOMAIN_CONNECTED || "").trim().toLowerCase() === "true";
  if (configured && customDomainConnected) return normalizeSiteOrigin(configured);
  try {
    return new URL(request?.url || configured || DEFAULT_SITE_ORIGIN).origin;
  } catch {
    return configured ? normalizeSiteOrigin(configured) : DEFAULT_SITE_ORIGIN;
  }
}
