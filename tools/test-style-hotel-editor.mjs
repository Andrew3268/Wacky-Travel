...
assert.match(read("public/add.html"), /add\.js\?v=20260921-hotel-review-json-v1/);
assert.match(read("public/edit.html"), /edit\.js\?v=20260921-hotel-review-json-v1/);
for (const file of ["public/add.html", "public/edit.html"]) {
  const html = read(file);
...
assert.equal((postRendererSource.match(/app\.css\?v=20260914-editor-tip-v2/g) || []).length, 2, "post render paths must both bust the immutable app.css cache");
