...
assert.match(renderer, /POST_RENDER_VERSION = "20260817-post-layout-v46"/);
...
const recommendedH3Rule = appCss.match(/body\.post-page-body--hotel-review-magazine \.post-body \.post-content h3\{([\s\S]*?)\}/)?.[1] || "";
...
assert.match(css, /body\.post-page-body \.post-author-profile__avatar\{[\s\S]*?width:42px;[\s\S]*?height:42px;[\s\S]*?border-radius:50%;[\s\S]*?object-fit:cover;/);
...
const aboutFooter = about.match(/<footer class="wtpromo-footer[\s\S]*?<\/footer>/)?.[0] || "";
const rendererFooter = renderer.match(/function footer\(siteName\) \{[\s\S]*?\n\}/)?.[0] || "";
assert.ok(!aboutFooter.includes('/about/#editorial-policy'));
assert.ok(!rendererFooter.includes('/about/#editorial-policy'));
assert.ok(!rendererFooter.includes('/hotel-promotions/'));
