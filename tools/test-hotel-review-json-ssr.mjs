...
assert.match(post, /hotel-review-json\.css\?v=20260926-location-summary-v20/);
...
assert.match(mapJs, /<span class="hrj-map-label__text">호텔<\/span>/);
assert.match(mapJs, /interactive: true/);
assert.match(mapJs, /hotelMarker\.on\("click"/);
assert.match(mapJs, /현재 숙소 기준점/);
assert.match(css, /\.hrj-map-overlay--hotel/);
...
assert.match(mapJs, /requestAnimationFrame\(\(\) => activateItem/);
...
assert.match(post, /POST_RENDER_VERSION = "20260926-post-layout-v68"/);
