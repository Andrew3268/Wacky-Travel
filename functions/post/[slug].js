...
const POST_RENDER_VERSION = "20260926-post-layout-v67";
...
export async function onRequestGet(context) {
  ...
  <link rel="preconnect" href="https://pix8.agoda.net" crossorigin />`
      ...
      const affiliateDisclosureHtml = affiliateDisclosure
        ? `<p class="post-affiliate-disclosure" role="note">${escapeHtml(affiliateDisclosure)}</p>`
        : "";
...
<head>
  ...
  ${isTravelTipPost
    ? `<link rel="stylesheet" href="/assets/css/post-public.css?v=20260914-editor-tip-v2" />`
    : `<link rel="stylesheet" href="/assets/css/app.css?v=20260914-editor-tip-v2" />
  ...
  ${isJsonHotelReviewPost ? `<link rel="stylesheet" href="/assets/css/hotel-review-json.css?v=20260926-location-map-padding-v19" />` : ""}
<style>
    .post-body,
...
<body class="${escapeHtml(bodyClassName)}">
  ...
  ${isJsonHotelReviewPost ? `<script defer src="/assets/js/hotel-review-map.js?v=20260926-location-map-padding-v19"></script>` : ""}
  ...
  ${isDraftPreview ? `<script src="/assets/js/admin-ui.js?v=20260721NoHeaderLogoutV2" defer></script>` : ""}
      ...
      const res = okHtml(html, {
        headers: {
          "cache-control": isDraftPreview ? "private, no-store" : "public, max-age=0, s-maxage=600, must-revalidate"
        }
      });
  ...
  return `
    <section class="post-hotel-key-points" aria-labelledby="postHotelKeyPointsTitle">
      ...
      <div class="post-hotel-key-points__grid">
        ${items.map((item) => `
          <div class="post-hotel-key-point" aria-label="${escapeHtml(item.label)}: ${escapeHtml(item.text)}">
            <span class="post-hotel-key-point__icon" aria-hidden="true">${renderHotelKeyPointIcon(item.key)}</span>
            <span class="post-hotel-key-point__sr-only">${escapeHtml(item.label)}</span>
