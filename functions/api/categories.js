...
export async function onRequestGet({ env }) {
  return okJson(
    { items: await getCategories(env.TRAVEL_DB) },
...
export async function onRequestPost({ env, request }) {
  ...
  await ensureCategoriesTable(env.TRAVEL_DB);
  const body = await request.json().catch(() => null);
  const name = normalizeCategoryName(body?.name);
  if (!name) return okJson({ message: "카테고리 이름을 입력하세요." }, { status: 400 });

  const exists = await env.TRAVEL_DB.prepare(`SELECT name FROM categories WHERE name = ?`).bind(name).first();
  if (exists) return okJson({ message: "같은 이름의 카테고리가 이미 있습니다." }, { status: 409 });

  const maxRow = await env.TRAVEL_DB.prepare(`SELECT COALESCE(MAX(sort_order), 0) AS max_sort FROM categories`).first();
  const now = new Date().toISOString();
  ...
  return okJson({ ok: true, item: { name }, items: await getCategories(env.TRAVEL_DB) });
}
...
export async function onRequestPut({ env, request }) {
  ...
  const current = await env.TRAVEL_DB.prepare(`SELECT name FROM categories WHERE name = ?`).bind(currentName).first();
  if (!current) return okJson({ message: "수정할 카테고리를 찾지 못했습니다." }, { status: 404 });

  if (currentName !== newName) {
    const duplicate = await env.TRAVEL_DB.prepare(`SELECT name FROM categories WHERE name = ?`).bind(newName).first();
  ...
  return okJson({ ok: true, item: { name: newName }, items: await getCategories(env.TRAVEL_DB) });
}
...
export async function onRequestDelete({ env, request }) {
  ...
  const current = await env.TRAVEL_DB.prepare(`SELECT name FROM categories WHERE name = ?`).bind(name).first();
  if (!current) return okJson({ message: "삭제할 카테고리를 찾지 못했습니다." }, { status: 404 });

  const now = new Date().toISOString();
  await env.TRAVEL_DB.prepare(`DELETE FROM categories WHERE name = ?`).bind(name).run();
  await env.TRAVEL_DB.prepare(`
    UPDATE posts
    SET category = '', updated_at = ?
    WHERE TRIM(COALESCE(category, '')) = ?
  ...
  return okJson({ ok: true, items: await getCategories(env.TRAVEL_DB) });
}
