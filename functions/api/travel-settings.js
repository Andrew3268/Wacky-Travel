...
async function getNextRecommendationCategorySortOrder(db) {
  const row = await db.prepare(`SELECT COALESCE(MAX(sort_order), 0) AS max_sort FROM recommendation_categories WHERE TRIM(COALESCE(destination_slug, '')) = ''`).first();
  return Number(row?.max_sort || 0) + 1;
}
...
function readRecommendationCategoryPayload(body, { current = null } = {}) {
  const name = readBodyField(body, "name", current?.name);
...
export async function onRequestPost({ env, request }) {
  ...
  if (entity === "country") {
    ...
    if (entity === "recommendation_category") {
      const payload = readRecommendationCategoryPayload(body);
      const slug = requireSlug(body.slug, payload.name);
      if (!payload.name) return okJson({ message: "추천 카테고리 이름을 입력해 주세요." }, { status: 400 });
      const exists = await env.TRAVEL_DB.prepare(`SELECT id FROM recommendation_categories WHERE TRIM(COALESCE(destination_slug, '')) = '' AND slug = ?`).bind(slug).first();
      if (exists) return okJson({ message: "같은 slug의 추천 카테고리가 이미 있습니다." }, { status: 409 });
      const sortOrder = payload.sortOrder || await getNextRecommendationCategorySortOrder(env.TRAVEL_DB);
      await env.TRAVEL_DB.prepare(`
        ...
        VALUES (?, ?, ?, '', '', ?, ?, ?, ?)
      `).bind(slug, payload.name, payload.description, sortOrder, payload.isActive, now, now).run();
      return okJson({ ok: true, ...(await getTravelSettings(env.TRAVEL_DB, { includeInactive: true })) });
    }
...
export async function onRequestPut({ env, request }) {
  ...
  if (entity === "recommendation_category") {
    const current = await env.TRAVEL_DB.prepare(`SELECT * FROM recommendation_categories WHERE TRIM(COALESCE(destination_slug, '')) = '' AND slug = ?`).bind(currentSlug).first()
      || await env.TRAVEL_DB.prepare(`SELECT * FROM recommendation_categories WHERE slug = ? ORDER BY CASE WHEN TRIM(COALESCE(destination_slug, '')) = '' THEN 0 ELSE 1 END, sort_order ASC LIMIT 1`).bind(currentSlug).first();
    if (!current) return okJson({ message: "수정할 추천 카테고리를 찾지 못했습니다." }, { status: 404 });
    const nextSlug = requireSlug(body.slug, currentSlug);
    const payload = readRecommendationCategoryPayload(body, { current });
    if (!payload.name) return okJson({ message: "추천 카테고리 이름을 입력해 주세요." }, { status: 400 });
    if (nextSlug !== currentSlug) {
      const duplicate = await env.TRAVEL_DB.prepare(`SELECT id FROM recommendation_categories WHERE TRIM(COALESCE(destination_slug, '')) = '' AND slug = ?`).bind(nextSlug).first();
      if (duplicate) return okJson({ message: "같은 slug의 추천 카테고리가 이미 있습니다." }, { status: 409 });
    }
    await env.TRAVEL_DB.prepare(`
      UPDATE recommendation_categories
      SET slug = ?, name = ?, description = ?, country_slug = '', destination_slug = '', sort_order = ?, is_active = ?, updated_at = ?
      WHERE id = ?
    `).bind(nextSlug, payload.name, payload.description, payload.sortOrder, payload.isActive, now, current.id).run();
    if (nextSlug !== currentSlug || payload.name !== current.name || payload.description !== current.description) {
      await env.TRAVEL_DB.prepare(`
        UPDATE posts
        SET recommendation_category_slug = ?, recommendation_category_name = ?, recommendation_category_description = ?, updated_at = ?
        WHERE recommendation_category_slug = ?
      `).bind(nextSlug, payload.name, payload.description, now, currentSlug).run();
    }
    return okJson({ ok: true, ...(await getTravelSettings(env.TRAVEL_DB, { includeInactive: true })) });
  }
...
export async function onRequestDelete({ env, request }) {
  ...
  if (entity === "recommendation_category") {
    const hardDelete = Number(body.hard_delete ?? 0) === 1 || String(body.action || body.mode || "").trim() === "delete";
    if (hardDelete) {
      await env.TRAVEL_DB.prepare(`UPDATE posts SET recommendation_category_slug = '', recommendation_category_name = '', recommendation_category_description = '', updated_at = ? WHERE recommendation_category_slug = ?`).bind(now, slug).run();
      await env.TRAVEL_DB.prepare(`DELETE FROM recommendation_categories WHERE slug = ?`).bind(slug).run();
      return okJson({ ok: true, ...(await getTravelSettings(env.TRAVEL_DB, { includeInactive: true })) });
    }
    await env.TRAVEL_DB.prepare(`UPDATE recommendation_categories SET is_active = 0, updated_at = ? WHERE slug = ?`).bind(now, slug).run();
    return okJson({ ok: true, ...(await getTravelSettings(env.TRAVEL_DB, { includeInactive: true })) });
  }
