...
const classNames = (value = []) => value.filter(Boolean).join(' ');
...
function renderAreaChoice(area) {
  const cards = area.choices.map((choice) => `<article class="${escapeHtml(classNames(choice.classes))}">
...
<p class="wt-choice-card__line">${escapeHtml(choice.line)}</p>
  ...
  return `<section aria-labelledby="${escapeHtml(area.ariaLabelledby)}" class="${escapeHtml(classNames(area.classes))}" id="area-compare">
<div class="container">
...
function renderReasonSection(section) {
  ...
  return `<section aria-labelledby="${escapeHtml(section.ariaLabelledby)}" class="${escapeHtml(classNames(section.classes))}">
<div class="container">
...
function renderSurvey(survey) {
  return `<section class="${escapeHtml(classNames(survey.classes))}">
<div class="container">
...
function renderBookingChecks(booking) {
  ...
  return `<section aria-labelledby="${escapeHtml(booking.ariaLabelledby)}" class="${escapeHtml(classNames(booking.classes))}">
<div class="container">
...
function renderHotelCard(card) {
  ...
  const cardAttrs = attrs({ 'aria-describedby': card.ariaDescribedby, class: 'wt-purpose-hotel-card', href: card.href, rel: card.rel, target: card.target });
...
function renderHotels(hotels) {
  const sectionAttrs = attrs({
    ...
    class: classNames(hotels.classes),
...
function renderOtherChoices(other) {
  ...
  return `<section aria-labelledby="${escapeHtml(other.ariaLabelledby)}" class="${escapeHtml(classNames(other.classes))}">
<div class="container">
...
function renderFaq(faq) {
  ...
  return `<section aria-labelledby="${escapeHtml(faq.ariaLabelledby)}" class="${escapeHtml(classNames(faq.classes))}">
<div class="container">
...
<p class="wtpromo-footer-copy">© 2026 Be Stayable<br/>예약 전 가격, 취소 조건, 세금 포함 여부를 다시 확인하세요.</p>
</div>
<nav aria-label="하단 메뉴" class="wtpromo-footer-links">
<a href="/about/">소개</a><a href="/about/#editorial-policy">편집 원칙</a>
...
<main class="${escapeHtml(classNames(page.mainClasses))}" data-purpose-template="fukuoka-v1">
${renderHero(page.hero)}
${renderAreaChoice(page.areaChoice)}
${page.reasons.map(renderReasonSection).join('\n')}
