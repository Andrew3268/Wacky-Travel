const cityConfig = {
  ...
  "areas": {
    "station": {
      ...
      "summary": "화렌역 주변은 타이베이·이란에서 기차로 도착한 뒤 체크인, 짐 보관, 택시 승하차를 가장 단순하게 처리하기 좋은 지역입니다.",
    ...
    "meilun": {
      "name": "메이룬·항구",
      "slug": "meilun-harbor",
      "summary": "메이룬과 화렌항 주변은 시내 중심보다 차분하고, 넓은 객실이나 가족형 호텔을 찾을 때 비교하기 좋은 지역입니다.",
    ...
    "taroko": {
      ...
      "summary": "타이루거와 톈샹 지역은 협곡과 산악 풍경을 중심에 두고 하루를 길게 쓰는 여행자에게 맞는 특수 목적형 지역입니다.",
...
(function () {
  ...
  function renderResult() {
    ...
    $('resultWhyText').textContent = `${top.name}은(는) 이번 답변에서 가장 높은 점수를 받은 지역입니다. ${second ? `차선책으로는 ${second.name}도 함께 비교해보세요.` : ''}`;
    const reasons = [
      ['이번 일정과 맞는 이유', top.compareGood || top.label || top.summary],
      ['예약 전 확인할 점', top.compareCaution || '숙소 주변 이동 방식과 밤 동선을 함께 확인하세요.'],
      ['비교해볼 대안', second ? `${second.name}은(는) 일정이 달라질 때 함께 볼 만한 대안입니다.` : '일정 성격이 다르다면 다른 화렌 지역도 함께 비교해보세요.']
    ];
